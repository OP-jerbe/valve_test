...
class MotorController:
    ...
    def set_microsteps_per_step(self, microsteps_per_step: int = 256) -> None:
        ...
        if microsteps_per_step not in acceptable_values:
            raise ValueError(
                f"{microsteps_per_step} is not an acceptable value. Acceptable values: {acceptable_values}"
            )
        else:
...
if __name__ == "__main__":
    ...
    from constants import MICROSTEPS_PER_REV, MICROSTEPS_PER_STEP  # type: ignore
